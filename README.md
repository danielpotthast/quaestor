...
| `DEFAULT_LANGUAGE`            | The language new users start with. Supported values are `en` and `de`. Each user can change it later in their settings. Unsupported values fall back to the default value.                                                                                                                                         | `en`                                                            |
| `DEFAULT_CURRENCY`            | The display currency new users start with. Supported values are `EUR`, `USD`, `GBP`, `CHF`, `JPY`, `CAD` and `AUD`. Selects only the symbol/format for amounts (Quaestor does not convert between currencies). Each user can change it later in their settings. Unsupported values fall back to the default value. | `EUR`                                                           |
